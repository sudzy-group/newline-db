...
SELECT min(oi.userId) as userId, min(c.categoryName) as categoryName, min(d.categoryId) as category, min(oi.name) as name, min(oi.dishId) as dishId, min(oi.createdAt) - mod(min(oi.createdAt), 60 * 60 * 1000) as milisec, CONVERT_TZ(FROM_UNIXTIME((min(oi.createdAt) - mod(min(oi.createdAt), 60 * 60 * 1000))/1000), '+00:00', '-12:00') as hour, sum(quantity) as sold, sum(quantity) * min(price) as gross FROM `OrderItems` as oi
...
SELECT min(oic.userId) as userId, min(oic.name) as name, min(oic.createdAt) - mod(min(oic.createdAt), 60 * 60 * 1000) as milisec, CONVERT_TZ(FROM_UNIXTIME((min(oic.createdAt) - mod(min(oic.createdAt), 60 * 60 * 1000))/1000), '+00:00', '-12:00') as hour, sum(quantity) as sold, sum(quantity) * min(price) as gross FROM `OrderItemChoices` as oic
